...
void main() {
   float size = 0.0005;
	...
	vec3 normal = vertex[0].normal.xyz;
	  ...
	  float b = - 2 * ny * nz;
	  float a1 = - pow(nx,2) - pow(nz,2);
	  float c1 = pow(nx,2) - pow(ny,2);

	  float a2 = - pow(nx,2) - pow(nz,2);
	  float c2 = -pow(nx,2) - pow(ny,2);
	 ...
	 if(delta1>0){
		z1 = (-b + sqrt(delta1))/(2*a1);
		z2 = (-b - sqrt(delta1))/(2*a1);
	 }else{
		z1 = (-b + sqrt(delta2))/(2*a2);
		z2 = (-b - sqrt(delta2))/(2*a2);
	 }
	
	 float x1 = sqrt(1 - pow(z1, 2));
	 float x2 = sqrt(1 - pow(z2, 2));
