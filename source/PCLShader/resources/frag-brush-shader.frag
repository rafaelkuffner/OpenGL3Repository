	...
	if(aBuffer){
		...
		if(coords.x>=0 && coords.y>=0 && coords.x<SCREEN_WIDTH && coords.y<SCREEN_HEIGHT ){
			...
			if(dualPaint && brushType != 3){
				...
				vec4 col2;
				if(brushType==2) col2 = sbrColor9Gaussian(a*1.2,gamma-0.3,sigmay,sigmay,0,saturation+0.4);
				if(brushType==0) col2 = sbrColor9Gaussian(a*1.2,gamma-0.3,sigmax/1.2,sigmay/2.5,0,saturation+0.4);
				if(brushType==1) col2 = sbrColor9Gaussian(a*1.2,gamma-0.3,sigmax/1.2,sigmay/2.5,0,saturation+0.4);
